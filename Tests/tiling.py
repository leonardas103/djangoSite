...
def split_square(image, overlap, sections):
...
def split_row(image, overlap, sections):
...
def merge_row(image, tiles, overlap, sections):
	height, _ = np.shape(image)
...
def merge_square(image, tiles, overlap, sections):
	height, width = np.shape(image)
	data_type = int if isinstance(tiles[0][0,:],int) else float	
	result = np.empty_like(image, dtype=data_type) 
	step = int(np.ceil(width/(sections//2)))
